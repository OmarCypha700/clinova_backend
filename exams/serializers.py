...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'username', 'email', 'first_name', 'last_name', 'role', 'school', 'school_name', 'is_active', 'date_joined']
        read_only_fields = ['date_joined', 'school']
...
class StudentCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_program_id(self, value):
        """Ensure program belongs to current school"""
        school = get_current_school()
        if school:
            if not Program.objects.filter(id=value, school=school).exists():
                raise serializers.ValidationError("Program not found in your school")
        return value
    
    def create(self, validated_data):
        program_id = validated_data.pop('program_id')
        validated_data['program_id'] = program_id
        # School will be auto-assigned by TenantModel
        return Student.objects.create(**validated_data)
...
class ProcedureStepCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    def validate_procedure_id(self, value):
        """Ensure procedure belongs to current school"""
        school = get_current_school()
        if school:
            if not Procedure.objects.filter(id=value, school=school).exists():
                raise serializers.ValidationError("Procedure not found in your school")
        return value
...
class ProcedureListSerializer(serializers.ModelSerializer):
    ...
    def get_status(self, obj):
        ...
        # Ensure we're only looking at procedures in current school
        sp = obj.studentprocedure_set.filter(student_id=student_id).first()
        if not sp:
            return "pending"
...
class CarePlanCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        # Ensure student and program belong to current school
        school = get_current_school()
        if school:
            if data['student'].school != school:
                raise serializers.ValidationError("Student not found in your school")
            if data['program'].school != school:
                raise serializers.ValidationError("Program not found in your school")
